...
class FacialRecognitionApp:
    ...
    # Resto de los métodos de la clase aquí
    ...
    def save_result(self):
        ...
        if current_result:
            person_id, image_path, confidence = current_result
    ...
    def get_current_result(self):
        ...
        confidence_str = self.result_label.cget("text")

        # Extraer el valor numérico de la confianza
        confidence_value = None
        try:
            confidence_value = float(confidence_str.split(":")[1].strip().strip('%'))
        except ValueError:
            messagebox.showerror("Error", "No se pudo extraer la confianza como un número.")
            return None

    # Validar si los valores son válidos
        if person_id and image_path and confidence_value is not None:
            return person_id, image_path, confidence_value
        else:
            return None
