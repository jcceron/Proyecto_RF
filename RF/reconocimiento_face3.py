...
class FacialRecognitionApp:
    def __init__(self, root):
        ...
        ''')
        # Button to show database records
        tk.Button(root, text="Show Database Records", command=self.show_database_records).grid(row=5, column=0, columnspan=2, pady=10)
    ...
    def compare_with_known_faces(self, unknown_face_encoding):
        ...
        # Get the person ID with the highest confidence
        top_person_id = results[0][0]

        # Update the person ID entry field
        self.person_id_var.set(top_person_id.split(".")[0])  # Remove file extension

        # Store the result with the highest confidence in the database
    ...
    def store_recognition_result_to_db(self, result):
        # Store the result with the highest confidence in the in-memory database
    ...
    def show_database_records(self):
        # Retrieve and display records from the in-memory database
    ...
    def display_database_records(self, records):
        ...
        tree.column("Person ID", anchor=tk.W, width=150)
        tree.heading("Image Path", text="Image Path")
        tree.column("Image Path", anchor=tk.W, width=300)
